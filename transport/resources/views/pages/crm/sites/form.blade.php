...
new #[Title('Site')] class extends Component {
    ...
    public function mount(CurrentCompany $currentCompany, ?Site $site = null): void { $this->company_id=$currentCompany->id(auth()->user()); abort_if($site && (int)$site->company_id !== $this->company_id, 404); $this->site = $site; foreach (['name','address_line_1','address_line_2','town','county','postcode','what_3_words','address_code','customer_id'] as $field) { $this->{$field} = $site?->{$field} ?? ($this->{$field} ?? null); } }
    public function save(CurrentCompany $currentCompany): mixed
    {
        ...
        $data = $this->validate(['company_id'=>['required','integer'],'customer_id'=>['nullable',Rule::exists('customers','id')->where('company_id',$this->company_id)],'name'=>['required','string','max:255'],'address_line_1'=>['required','string','max:255'],'address_line_2'=>['nullable','string','max:255'],'town'=>['nullable','string','max:255'],'county'=>['nullable','string','max:255'],'postcode'=>['required','string','max:20'],'what_3_words'=>['nullable','string','max:255'],'address_code'=>['nullable','string','max:255',Rule::unique('sites')->ignore($this->site?->id)]]);
