...
new #[Title('Equipment')] class extends Component
{
    ...
    public function createModel(): void
    {
        ...
        $this->reset(['new_make_id', 'new_model_name']);
        Flux::modal('create-model')->close();
        Flux::toast(text: 'Model created', variant: 'success');
    }

    public function mount(CurrentCompany $currentCompany, ?Equipment $equipment = null): void
    {
        $this->company_id = $currentCompany->id(Auth::user());
        abort_if($equipment && (int) $equipment->company_id !== $this->company_id, 404);
        $this->equipment = $equipment;
        $this->model_id = $equipment?->model_id;
        $this->make_id = $equipment?->equipmentModel?->make_id;
        $this->stock_number = $equipment?->stock_number ?? '';
        $this->serial_number = $equipment?->serial_number ?? '';
        $this->fleet_number = $equipment?->fleet_number;
        $this->customer_id = $equipment?->customers()->value('customers.id');
        $this->owner_type = $this->customer_id ? 'customer' : 'tenant';
        $this->owner_company_id = $equipment?->company_id ?? $this->company_id;
    }
