...
<section class="w-full p-6">
    ...
    <x-pages::setup.layout contentclass="max-w-5xl">
        <div class="mb-4 flex justify-between gap-3"><flux:input class="max-w-sm flex-1" wire:model.live.debounce.300ms="search" icon="magnifying-glass" placeholder="Search sites..." />@can('setup.site.create')<flux:button variant="primary" :href="route('setup.sites.create')">Add Site</flux:button>@endcan</div>
        <flux:table :paginate="$sites">
            ...
            <flux:table.rows>
                @forelse ($sites as $site)
                    <flux:table.row :key="$site->id"><flux:table.cell>{{ $site->name }}</flux:table.cell><flux:table.cell>{{ $site->town ?: '—' }}</flux:table.cell><flux:table.cell>{{ $site->postcode }}</flux:table.cell><flux:table.cell>{{ $site->what_3_words ?: '—' }}</flux:table.cell><flux:table.cell>@can('setup.site.update')<flux:button size="sm" variant="ghost" :href="route('setup.sites.update',$site)">Manage</flux:button>@endcan</flux:table.cell></flux:table.row>
                @empty
