...
new #[Title('Site')] class extends Component {
    ...
    {
        Gate::authorize($this->site ? 'setup.site.update' : 'setup.site.create');
        ...
        Flux::toast(text: 'Site saved successfully', variant: 'success'); return $this->redirectRoute('setup.sites.index', navigate: true);
    }
};
?>
<section class="w-full p-6"><flux:heading size="xl">{{ $site ? __('Edit Site') : __('Create Site') }}</flux:heading><x-pages::setup.layout contentclass="max-w-3xl"><form wire:submit="save" class="grid gap-5 sm:grid-cols-2"><flux:input wire:model="name" label="Name" required /><flux:input wire:model="address_code" label="Address code" /><flux:input wire:model="address_line_1" label="Address line 1" required /><flux:input wire:model="address_line_2" label="Address line 2" /><flux:input wire:model="town" label="Town" /><flux:input wire:model="county" label="County" /><flux:input wire:model="postcode" label="Postcode" required /><flux:input wire:model="what_3_words" label="What3Words" /><div class="flex justify-end gap-3 sm:col-span-2"><flux:button :href="route('setup.sites.index')" variant="ghost">{{ __('Cancel') }}</flux:button><flux:button type="submit" variant="primary">{{ __('Save') }}</flux:button></div></form></x-pages::setup.layout></section>
