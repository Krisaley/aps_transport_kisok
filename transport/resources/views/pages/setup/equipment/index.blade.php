...
<section class="w-full p-6">
    ...
    <x-pages::setup.layout contentclass="max-w-5xl">
        <div class="mb-4 flex justify-between gap-3"><flux:input class="max-w-sm flex-1" wire:model.live.debounce.300ms="search" icon="magnifying-glass" placeholder="Search equipment..." />@can('setup.equipment.create')<flux:button variant="primary" :href="route('setup.equipment.create')">Add Equipment</flux:button>@endcan</div>
        <flux:table :paginate="$equipment">
            ...
            <flux:table.rows>
                @forelse ($equipment as $item)
                    <flux:table.row :key="$item->id"><flux:table.cell>{{ $item->stock_number }}</flux:table.cell><flux:table.cell>{{ $item->equipmentModel->make->name }}</flux:table.cell><flux:table.cell>{{ $item->equipmentModel->name }}</flux:table.cell><flux:table.cell>{{ $item->serial_number }}</flux:table.cell><flux:table.cell>@can('setup.equipment.update')<flux:button size="sm" variant="ghost" :href="route('setup.equipment.update',$item)">Manage</flux:button>@endcan</flux:table.cell></flux:table.row>
                @empty
