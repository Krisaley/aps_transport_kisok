...
new #[Title('Equipment')] class extends Component {
    ...
    {
        Gate::authorize($this->equipment ? 'setup.equipment.update' : 'setup.equipment.create');
        ...
        Flux::toast(text:'Equipment saved successfully',variant:'success'); return $this->redirectRoute('setup.equipment.index',navigate:true);
    }
    public function with(): array { return ['models'=>EquipmentModel::query()->with('make')->orderBy('name')->get()]; }
};
?>
<section class="w-full p-6"><flux:heading size="xl">{{ $equipment ? __('Edit Equipment') : __('Create Equipment') }}</flux:heading><x-pages::setup.layout contentclass="max-w-3xl"><form wire:submit="save" class="space-y-6"><flux:select wire:model="model_id" label="Model"><flux:select.option value="">{{ __('Select a model') }}</flux:select.option>@foreach($models as $model)<flux:select.option :value="$model->id">{{ $model->make->name }} — {{ $model->name }}</flux:select.option>@endforeach</flux:select><flux:input wire:model="stock_number" label="Stock number" required /><flux:input wire:model="serial_number" label="Serial number" required /><div class="flex justify-end gap-3"><flux:button :href="route('setup.equipment.index')" variant="ghost">{{ __('Cancel') }}</flux:button><flux:button type="submit" variant="primary">{{ __('Save') }}</flux:button></div></form></x-pages::setup.layout></section>
