...
<section class="w-full p-6">
    ...
    <x-pages::setup.layout contentclass="max-w-5xl">
        <div class="mb-4 flex gap-2"><flux:input class="max-w-sm flex-1" wire:model.live.debounce.300ms="search" icon="magnifying-glass" placeholder="Search vehicles..." /><flux:select wire:model.live="statusFilter"><flux:select.option value="all">{{ __('All') }}</flux:select.option><flux:select.option value="active">{{ __('Active') }}</flux:select.option><flux:select.option value="inactive">{{ __('Inactive') }}</flux:select.option></flux:select>@can('setup.vehicle.create')<flux:button variant="primary" :href="route('setup.vehicles.create')">Add Vehicle</flux:button>@endcan</div>
        <flux:table :paginate="$vehicles">
            ...
            <flux:table.rows>
                @forelse ($vehicles as $vehicle)
                    <flux:table.row :key="$vehicle->id"><flux:table.cell>{{ $vehicle->is_active ? __('Active') : __('Inactive') }}</flux:table.cell><flux:table.cell>{{ $vehicle->name }}</flux:table.cell><flux:table.cell>{{ $vehicle->registration }}</flux:table.cell><flux:table.cell>@can('setup.vehicle.update')<flux:button size="sm" variant="ghost" :href="route('setup.vehicles.update',$vehicle)">Manage</flux:button>@endcan</flux:table.cell></flux:table.row>
                @empty
